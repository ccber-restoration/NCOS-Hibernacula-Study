...
p2 = ggplot(detections_by_habitat_species_pt_2, 
  ...
  scale_y_continuous(breaks = seq(0, 10, by = 2)) +  theme_minimal(base_family = "sans") +
  theme(
...
plots <- lapply(seq_along(feature_types), function(i) {
  ...
  p <- ggplot(
    ...
    geom_jitter(aes(fill = species_fill), color = "black", shape = 21, width = 0.25, alpha = .85, size = 5, show.legend = TRUE) +
    geom_errorbar(
...
map_main <- ggplot() +
  ...
  # Boundary with legend
  geom_sf(data = open_space_boundary, 
          aes(linetype = "NCOS Boundary"),  # <- mapped aesthetic
...
# Relabel study parts
  ...
  scale_linetype_manual(name = "", values = c("NCOS Boundary" = "solid")) +  # <- legend entry
  ...
  guides(
    ...
    linetype = guide_legend(override.aes = list(color = "yellow", linewidth = 2)) # <- styled legend
  )
...
t1_hourly <- species_final_hourly %>%
  ...
  add_header_above(c(" " = 1, "Constructed Hibernacula" = 3, "Boulders" = 3, "Logs" = 3, "Total" = 3))
...
metric_label_map <- c(
  detections_hib = "Hrs. Detected",  pct_hib = "% Visited",  rate_hib = "Per Cam.-Day",
  detections_bld = "Hrs. Detected",  pct_bld = "% Visited",  rate_bld = "Per Cam.-Day",
  detections_log = "Hrs. Detected",  pct_log = "% Visited",  rate_log = "Per Cam.-Day",
  detections_tot = "Hrs. Detected",  pct_tot = "% Visited",  rate_tot = "Per Cam.-Day"
)
...
feature_header <- c(" " = 1, build_feature_spans(1), build_feature_spans(2))

n_part1 <- sum(grepl("^part_1__", col_keep))
n_part2 <- sum(grepl("^part_2__", col_keep))
top_header <- c(" " = 1, if (n_part1 > 0) c("Winter" = n_part1), if (n_part2 > 0) c("Spring" = n_part2))
...
t1_hourly <- display_tbl %>%
  kable(
    ...
    align = c("l", rep("c", ncol(display_tbl) - 1)),
    digits = 3,
    escape = FALSE
  ) %>%
  ...
  column_spec((1 + n_part1), border_right = TRUE)
