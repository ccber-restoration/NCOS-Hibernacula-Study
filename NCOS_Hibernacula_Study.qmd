...
map_rgb_df$hex <- rgb(map_rgb_df$red, map_rgb_df$green, map_rgb_df$blue, maxColorValue = 255)
  ...
  study_part_shapes <- c("1" = 21, "2" = 24, "both" = 22)
...
map_main <- ggplot() +
  ...
  geom_sf(data = ncos_habitats_filtered, aes(fill = habitat_simple), color = NA, alpha = 0.5) +
  scale_fill_manual(values = habitat_colors, name = "Habitat Type",
                  guide = guide_legend(order = 1)) +  # Add this
  ...
  # Add deployment points on top
  ggnewscale::new_scale_fill() +
  geom_sf(data = map_site_sf, aes(fill = feature_type_methodology, shape = study_part_combined),
          color = "black", size = 4, alpha = 0.9, stroke = 0.8) +
...
theme(
  ...
  legend.box.margin = margin(t = -20, r = 0, b = 0, l = 0),  # Negative top margin pulls legend closer
  plot.margin = margin(t = 5, r = 5, b = 5, l = 5),  # Reduce overall plot margins
  axis.title = element_blank(),
  axis.text = element_blank(),
  axis.ticks = element_blank(),
...
# Force legend order with vertical stacking within each legend
guides(
  fill = guide_legend(order = 2, override.aes = list(alpha = 0.9, shape = 21, color = "black", stroke = 0.8, size = 4)),
  shape = guide_legend(order = 3, override.aes = list(size = 4, fill = "black", color = "black", stroke = 0.8)),
  linetype = guide_legend(order = 4, override.aes = list(color = "yellow", linewidth = 2))
)
